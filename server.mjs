...
async function api(req, res, url) {
  ...
  if (url.pathname.startsWith("/api/messages/") && req.method === "GET") {
    ...
    values.reverse(); const active = typing.get(target); return json(res, 200, { messages: values, hasMore: values.length >= 60, readThrough: Date.now(), typing: active?.until > Date.now() ? [active.name] : [], allowedReactions });
  }
  ...
  if (url.pathname === "/api/read" && req.method === "POST") { const input = await body(req); const target = Number(String(input.conversationId).split(":").slice(1).join(":")); await tg.readHistory(target); return json(res, 200, { ok: true }); }
